...
const ViewPlaylist = (props) => {
    ...
    useEffect(() => {
        async function fetchSamplePlaylist() {
            axinstance.get(`sampleplaylist/view/${id}`)
                .then((res) => {
    ...
    const handleDownload = () =>{
        ...
        axinstance.get(
            ...
            toast.error("An Error Occurred Trying to Download Playlist");
        ...
        <Container className="pt-4">
            {sloading || loading ?
                ...
                <>
                    ...
                    <p> Created By: <Link to={`/user/view/${samplePlaylist.username}`} > {samplePlaylist.username} </Link> </p>
                    {
                        !!user ?
                        <div>
                            
                        <Button variant="outline-dark" onClick={() => handleDownload()}> Download Playlist </Button>
                        {
                            downloadSpin ? <MyLoader> </MyLoader> :
                        showDownload ?  
                        <div> 
                            <a href={myLink} download={`${samplePlaylist.samplePlaylistName}.zip`}> File Download </a> 
                        </div> 
                        : 
                        <></>
                        }
